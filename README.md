...
前台页面：大厅 `/`、文章 `/post/:id`、归档 `/archive`、友链 `/links`、说说 `/murmur`、相册 `/album`、关于 `/about`、个人博客页 `/u/{用户名}`、注册 `/register`

账号体系：
- 管理员：admin / admin123（首次启动自动创建）
- 普通用户：通过邀请码注册（邀请码在管理员后台「邀请码」页面生成）
...
- 评论系统：**仅登录用户可评论**（未登录点击评论跳转登录页），账号身份自动识别、无需填写任何信息；楼中楼回复、emoji 表情面板；**默认直接通过展示**，后台可退回（前台隐藏）/ 标垃圾 / 删除；评论数统计
...
**第三阶段 · 多用户平台**
- 邀请码注册（注册即登录，管理员后台生成/管理邀请码）
- 角色体系：ADMIN 管理员（保留全部管理能力）/ USER 普通用户（个人中心，仅管理自己的内容）
- 首页即大厅：展示所有「开放」文章流（含作者信息），点击作者进入个人博客页 `/u/{用户名}`
- 文章可见性：开放（大厅/个人页可见）或仅自己可见（仅作者本人可看）
- 资源隔离：普通用户只能编辑/删除自己的文章与文件，管理员接口权限隔离
- 独立存储空间：每用户独立目录 `uploads/{用户ID}/`，默认 1GB 配额（上传前校验 + 用量查询）
